...
def parse_args():
    parser = argparse.ArgumentParser(description="Disable power to selected cameras on all connected sensors")
    parser.add_argument(
        "--mask",
        type=lambda x: int(x, 0),  # supports hex (e.g., 0xFF) or decimal
        default=0xFF,
        help="Camera bitmask to power off (default 0xFF for all cameras)",
    )
